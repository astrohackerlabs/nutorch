...
pub(crate) fn add_command_context(engine_state: EngineState) -> EngineState {
    ...
    nutorch::tensor::add_context(nu_explore::add_explore_context(engine_state))
}
...
mod tests {
    ...
    // Native module exports must remain covered even while out of command scope.
    fn audit_decls(ctx: &EngineState) -> Vec<(Vec<u8>, nu_protocol::DeclId)> {
        let mut decls = ctx.get_decls_sorted(true);
        let module = ctx.get_module(ctx.find_module(b"torch", &[]).unwrap());
        let compatibility = ctx.get_module(module.submodules[b"nutorch".as_slice()]);
        for id in module
            .decls
            .values()
            .chain(compatibility.decls.values())
            .copied()
            .chain(module.main)
        {
            decls.push((ctx.get_decl(id).name().as_bytes().to_vec(), id));
        }
        decls
    }
